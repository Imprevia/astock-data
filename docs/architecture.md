...
- 行业资金逐日序列只由 push2his 或其日序列缓存提供；无日期参数的 `getbkzj` f164 只表达本地当天五日累计，独立于逐日记录。
...
- f164 仅在今日五日窗口存在缺失 push2his 序列时整次服务调用批量获取一次，按板块代码精确匹配；有效逐日资金序列至少含一个非布尔数值 `main_net_inflow`，且该 eligible 路径不会因单板块异常跳过后续板块的 push2his 请求。缓存按目标日期精确读取，THS 行情字段不得升级为资金事实。
