...
Asserts the 26 public functions (``resolve_ticker`` + 25 ``get_*``) are
...
# The 25 public get_* entrypoints (sorted for stable diffs).
GET_FUNCS = [
    "get_balance_sheet",
    "get_cashflow",
    "get_concept_blocks",
...
PUBLIC_FUNCS = ["resolve_ticker", *GET_FUNCS]  # 26 total


# --------------------------------------------------------------------------- #
# __all__ sizing
# --------------------------------------------------------------------------- #
def test_api_all_has_exactly_26_names():
    assert len(api.__all__) == 26
    assert set(api.__all__) == set(PUBLIC_FUNCS)


def test_services_all_has_exactly_25_names():
    assert len(services.__all__) == 25
...
# --------------------------------------------------------------------------- #
def test_all_26_importable_from_top_level():
...
def test_all_26_importable_from_api_module():
    for name in PUBLIC_FUNCS:
...
def test_get_funcs_and_resolver_importable_from_services_and_resolver():
    ...
    # and re-exported at top-level alongside the 25 data functions
...
def test_every_get_func_returns_basemodel_subclass():
    """Each of the 25 get_* must annotate its return as a BaseModel subclass."""
...
def test_order_book_public_api_exposes_exact_cancellation_capability() -> None:
    from astock_data.api import get_order_book
    ...
    class FakeTencent:
        def order_book(self, code: str) -> dict:
            ...
            return {
                ...
                "last_price": 10.0,
                "bids": [{"position": 1, "price": 9.99, "volume_lots": 100}],
                "asks": [{"position": 1, "price": 10.01, "volume_lots": 120}],
                "bid_depth_lots": 100,
                "ask_depth_lots": 120,
                "spread": 0.02,
                "imbalance": -20 / 220,
    ...
    assert result.model_dump(mode="json")["exact_cancellation_available"] is False
