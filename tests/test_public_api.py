...
Asserts the 18 public functions (``resolve_ticker`` + 17 ``get_*``) are
...
# The 17 public get_* entrypoints (sorted for stable diffs).
GET_FUNCS = [
    "get_balance_sheet",
    "get_cashflow",
    "get_concept_blocks",
...
PUBLIC_FUNCS = ["resolve_ticker", *GET_FUNCS]  # 18 total


# --------------------------------------------------------------------------- #
# __all__ sizing
# --------------------------------------------------------------------------- #
def test_api_all_has_exactly_18_names():
    assert len(api.__all__) == 18
    assert set(api.__all__) == set(PUBLIC_FUNCS)


def test_services_all_has_exactly_17_names():
    assert len(services.__all__) == 17
...
# --------------------------------------------------------------------------- #
def test_all_18_importable_from_top_level():
...
def test_all_18_importable_from_api_module():
    for name in PUBLIC_FUNCS:
...
def test_get_funcs_and_resolver_importable_from_services_and_resolver():
    ...
    # and re-exported at top-level as the 18th public function
...
def test_every_get_func_returns_basemodel_subclass():
    """Each of the 17 get_* must annotate its return as a BaseModel subclass."""
